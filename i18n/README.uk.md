...
- **Транспорти**: stdio та HTTP/SSE, обидва підтримуються нативно через `rmcp`
...
Для мережевих клієнтів, що підключаються по HTTP/SSE:

```bash
roxy --transport http --port 8080 --upstream http://localhost:8000/mcp
# Клієнт підключається до http://localhost:8080/sse
```
...
| `--upstream-header <HEADER>` | — | Довільний HTTP-заголовок, `Name: Value`. Можна вказувати кілька разів |
...
MCP-клієнт (Claude, Cursor, Zed, ...)
       │
       │ JSON-RPC по stdio або HTTP/SSE
       ▼
┌──────────────┐
