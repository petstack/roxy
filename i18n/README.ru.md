...
- **Транспорты**: stdio и HTTP/SSE, оба поддерживаются нативно через `rmcp`
...
Для сетевых клиентов, подключающихся по HTTP/SSE:

```bash
roxy --transport http --port 8080 --upstream http://localhost:8000/mcp
# Клиент подключается к http://localhost:8080/sse
```
...
| `--upstream-header <HEADER>` | — | Произвольный HTTP-заголовок, `Name: Value`. Можно указывать несколько раз |
...
MCP-клиент (Claude, Cursor, Zed, ...)
       │
       │ JSON-RPC по stdio или HTTP/SSE
       ▼
┌──────────────┐
