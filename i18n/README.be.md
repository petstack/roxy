...
- **Транспарты**: stdio і HTTP/SSE, абодва падтрымліваюцца нативна праз `rmcp`
...
Для сеткавых кліентаў, якія падключаюцца па HTTP/SSE:

```bash
roxy --transport http --port 8080 --upstream http://localhost:8000/mcp
# Кліент падключаецца да http://localhost:8080/sse
```
...
MCP-кліент (Claude, Cursor, Zed, ...)
       │
       │ JSON-RPC па stdio або HTTP/SSE
       ▼
┌──────────────┐
