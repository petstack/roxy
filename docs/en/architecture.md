...
| [`src/transport.rs`](https://github.com/petstack/roxy/blob/main/src/transport.rs) | Builds the Streamable-HTTP service, configured to answer every MCP revision — sessions for the legacy ones, stateless for `2026-07-28` — and to apply roxy's inbound policies (`Host` allow-list, request-body cap). |
