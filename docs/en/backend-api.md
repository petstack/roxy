...
|---|---|---|
| `discover` | On every `list_*` request | Ask the backend to list all tools, resources, and prompts. |
...
**Your order does not matter — roxy imposes its own.** Return your tools, resources and prompts in whatever order is convenient (a directory scan, an unordered registry, a `SELECT` with no `ORDER BY`); roxy sorts them before they reach the client. Tools and prompts go by `name`, resources by `uri`, compared bytewise — so `DATA_EXPORT` sorts before `getUser`, because uppercase letters come first in ASCII. That is not roxy being broken; it is the property that makes the order reproducible.

Why it is done for you: the tool list ends up inside the model's prompt, so a list that shuffles between calls invalidates the client's prompt cache every time — and roxy re-runs `discover` on every listing, so an incidental order would shuffle often.

The trade-off, stated plainly: if you were curating tool order to steer which tool the model reaches for first, that lever is gone, and there is no flag to turn the sorting off. Use `name` and `description` to steer instead.
